import type { BracketResult, BracketChartBar } from "../types";

export function bracketsToChart(
  bracket: BracketResult,
  fullTable: { rate: number; lower: number; upper: number }[]
): BracketChartBar[] {
  return fullTable.map((row) => {
    ...
    // Active bracket: partially filled
      ...
      const usedInBracket =
        Math.max(0, Math.min(bracket.usedInThisBracket, width));
