// src/logic/adapters/lifetimeToChart.ts
import type {
  LifetimeSavingsResult,
  LifetimeTaxesDatum,
} from "../types";
 ...
 */
export function lifetimeToChart(result: LifetimeSavingsResult): LifetimeTaxesDatum[] {
  ...
  return baselineYears.map((baseYear, idx) => {
    ...
    const baseTotal =
      (baseYear.federalTax ?? 0) + (baseYear.irmaaSurchargeAnnual ?? 0);

    const rothTotal =
      (rothYear.federalTax ?? 0) + (rothYear.irmaaSurchargeAnnual ?? 0);
