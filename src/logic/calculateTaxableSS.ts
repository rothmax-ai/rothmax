...
import type { DomainInput, TaxableSSResult } from "./types";

/* ---------------------------------------------------------------
 * 1. SHARED INPUT SHAPE FOR ENGINE + TESTS
 * ---------------------------------------------------------------*/
export interface TaxableSSInput {
  filingStatus: "single" | "mfj" | "mfs" | "hoh";
  ordinaryIncome: number;        // NON-SS income
  socialSecurityGross: number;   // Full annual SS benefit
  taxExemptInterest: number;     // muni bond interest
}
...
export function calculateTaxableSS({
  ...
  socialSecurityGross,
  taxExemptInterest,
}: TaxableSSInput): number {
  if (socialSecurityGross <= 0) return 0;
 ...
 * ---------------------------------------------------------------*/
export function calculateTaxableSSFromDomainInput(
  input: DomainInput
): TaxableSSResult {
