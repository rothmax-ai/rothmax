 ...
 * inflation.ts
 * ------------
 * Helpers for inflating IRS tables over time for the projection engine.
 *
 * MVP behavior:
 *  - Assumes a constant inflationRate (from ProjectionInput)
 *  - Applies (1 + inflationRate) ^ yearIndex to:
 *      • Ordinary tax brackets (lower / upper bounds)
 *      • Standard deduction
 *
 * 100% pure logic — no React, no UI.
 */

type FilingStatus = "single" | "mfj" | "mfs" | "hoh";   // ✅ FIXED
import { brackets2025, standardDeduction2025 } from "../taxData/2025/brackets2025";

export interface InflatedBracketRow {
  rate: number;
  lower: number;
  upper: number;
}
 ...
 */
export function inflationMultiplier(
 ...
 */
export function getInflatedBrackets(
  ...
  return baseTable.map((row) => {
    ...
    const upper =
      row.upper === Infinity ? Infinity : Math.round(row.upper * mult);
 ...
 */
export function getInflatedStandardDeduction(
  filingStatus: FilingStatus,
  yearIndex: number,
  inflationRate: number
): number {
