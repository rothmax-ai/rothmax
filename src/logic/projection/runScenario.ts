...
function computeMarginalFromTable(
  taxableIncome: number,
  table: InflatedBracketRow[]
) {
...
function computeRMDForAge(
  age: number,
  iraBalance: number,
  rmdStartAge: number
) {
  if (age < rmdStartAge || iraBalance <= 0) return 0;
...
export function runScenario(input: ProjectionInput): YearProjection[] {
  const {
    filingStatus,
    currentAge,
    ssStartAge,
    ...
    socialSecurityAnnual: ss0,

    growthRate,
    taxableYieldRate,
    inflationRate,

    annualConversionPlan,
    applyIRMAA,
  } = input;
  ...
  for (
    ...
    if (conversion > maxConvertible) conversion = maxConvertible;
    ...
    const ordinaryIncome =
      wages + pension + rmd + conversion + interest + dividends + other;
    ...
    const bracketTable = getInflatedBrackets(
      filingStatus,
      yearIndex,
      inflationRate
    );

    const stdDeduction = getInflatedStandardDeduction(
      filingStatus,
      yearIndex,
      inflationRate
    );
    ...
    const { marginalRate, bracketIndex } =
      computeMarginalFromTable(taxableIncome, bracketTable);
    ...
    ──────────────────────────────────────────────*/
    if (ssActive) ssAnnual *= 1 + inflationRate;
  }
