...
// Public API
// ===============================================================
export function calculateLifetimeSavings(baselineInput, rothInput) {
 ...
 */
function calculateFederalTaxProgressive(taxableIncome, filingStatus) {
    if (taxableIncome <= 0)
        return 0;
 ...
 */
function calculateRmdForYear(age, iraStartOfYear, rmdStartAge) {
    if (age < rmdStartAge)
        return 0;
    if (iraStartOfYear <= 0)
        return 0;
...
function runScenario(input) {
    const { filingStatus, age, iraBalance, rothBalance, taxableBalance, lifeExpectancy, growthRate, taxableYieldRate, currentYear, agi, deductionAmount, taxExemptInterest, rothConversionAmount = 0, rmdStartAge, } = input;
    let ira = iraBalance;
    ...
    for (let a = age; a <= lifeExpectancy; a++) {
        ...
        const federalTax = calculateFederalTaxProgressive(taxableIncome, filingStatus);
        // -----------------------------------------------------------
        // 4. IRMAA (MAGI-based)
