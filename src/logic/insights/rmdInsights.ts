import type { RMDSchedule, RMDYear, DomainInput } from "../types";
import { formatCurrency } from "../../utils";

export type RmdInsightSeverity = "info" | "warn";

export interface RmdInsight {
  id: string;
  severity: RmdInsightSeverity;
  icon: string;

  label: string;            // short UI heading
  value: string | number;   // primary insight metric
  detail?: string;          // optional explanation
  text?: string;            // long-form fallback
}

export function generateRmdInsights(
  baseline: RMDSchedule,
  strategy: RMDSchedule,
  domain: DomainInput
): RmdInsight[] {
  const insights: RmdInsight[] = [];
  const { age, rmdStartAge } = domain;

  const baseYears = baseline.schedule ?? [];
  const stratYears = strategy.schedule ?? [];

  if (baseYears.length === 0) return insights;
  ...
  const peakBase =
    baseYears.reduce((m, y) => (y.rmd > m.rmd ? y : m), firstBase ?? baseYears[0]);
  ...
  if (age > rmdStartAge && currentBase) {
    insights.push({
      ...
      detail: `This year’s RMD at age ${age} is approximately ${formatCurrency(
        currentBase.rmd
      )}.`,
  ...
  if (peakBase && peakBase.rmd > 0) {
    insights.push({
      ...
      text: `Peak RMD is projected around age ${peakBase.age} at ${formatCurrency(
        peakBase.rmd
      )}.`,
    });
  }
  ...
  insights.push({
    ...
    detail: `Under your Roth conversion strategy, total lifetime RMDs fall from ${formatCurrency(
      totalBase
    )} to ${formatCurrency(totalStrat)} — a reduction of ${formatCurrency(
      lifetimeReduction
    )}.`,
    text: `Your Roth strategy reduces total lifetime RMDs by ${formatCurrency(
      lifetimeReduction
    )}.`,
  });
  ...
  if (age < rmdStartAge && domain.iraBalance > 0) {
    insights.push({
      ...
      value: "Opportunity",
      detail:
        "Because RMDs have not yet begun, you may have a strategic window to reduce future RMDs through Roth conversions.",
      text:
        "Because your RMDs haven’t started yet, you have a planning window to reduce future tax burden.",
    });
  }
