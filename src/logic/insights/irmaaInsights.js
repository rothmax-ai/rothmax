 ...
 */
export function generateIRMAAInsights(chart, vars = {}) {
        ...
        if (firstTierThreshold) {
            ...
            if (room > 0) {
                insights.push({
                    ...
                    severity: "info",
                    icon: "🩺",
                    text: `You are in the baseline Medicare tier (no IRMAA). You have ${formatCurrency(room)} of room before the first IRMAA tier.`
                });
            }
        }
    }
    ...
    if (!isBaseline && gap && gap > 10_000) {
        insights.push({
            ...
            severity: "info",
            icon: "🟦",
            text: `You have ${formatCurrency(gap)} of room before reaching the next IRMAA tier.`
        });
    }
    // ============================================================
    ...
    if (nextThreshold !== undefined) {
        ...
        if (gap2 > 0) {
            const warnLimit = userMAGI < 200_000
                ? 2500
                : userMAGI < 400_000
                    ? 5000
                    : 10_000;
            if (gap2 <= warnLimit) {
                insights.push({
                    ...
                    severity: "warn",
                    icon: "⚠️",
                    text: `You are ${formatCurrency(gap2)} below the next IRMAA tier. Even modest income changes could increase Medicare premiums.`
                });
            }
        }
    }
    ...
    if (prevThreshold !== undefined && difference !== undefined) {
        if (difference > 0 && difference <= 5000 && age >= 62) {
            insights.push({
                ...
                severity: "warn",
                icon: "⬇️",
                text: `You are ${formatCurrency(difference)} above the lower IRMAA tier. If your MAGI decreases by this amount (based on Medicare’s 2-year lookback), you may qualify for lower premiums.`
            });
        }
    }
    // ============================================================
    ...
    if (currentTierIndex === thresholds.length - 1) {
        insights.push({
            ...
            severity: "danger",
            icon: "🚨",
            text: "You are in the highest IRMAA tier. Additional income will not increase Medicare surcharges further under current rules."
        });
    }
    // ============================================================
    ...
    if (age >= 62 || insights.length > 0) {
        insights.push({
            ...
            severity: "info",
            icon: "📅",
            text: `Medicare uses income from two years ago to determine IRMAA. Your current-year MAGI will affect Medicare premiums in ${currentYear + 2}.`
        });
    }
    // 7. Roth conversion impact
    if (vars.plannedConversion &&
        nextThreshold &&
        ...
        insights.push({
            ...
            severity: "warn",
            icon: "🔥",
            text: `A Roth conversion above ${formatCurrency(gapNeeded)} this year would place you into a higher IRMAA tier in ${currentYear + 2}.`
        });
    }
    // 8. Capital gains impact
    if (vars.capitalGains &&
        nextThreshold &&
        ...
        insights.push({
            ...
            severity: "warn",
            icon: "💸",
            text: `Realizing capital gains of more than ${formatCurrency(gapNeeded)} could move you into a higher Medicare premium tier.`
        });
    }
    ...
    if (vars.taxExemptInterest && vars.taxExemptInterest > 0) {
        insights.push({
            ...
            severity: "info",
            icon: "🏦",
            text: `Medicare includes tax-exempt interest in IRMAA calculations. Your tax-exempt income of ${formatCurrency(vars.taxExemptInterest)} counts toward your tier.`
        });
    }
    // 10. Pension income
    ...
    if (vars.iraBalance &&
        ...
        insights.push({
            ...
            severity: "warn",
            icon: "⏳",
            text: `Your future RMDs starting at age ${vars.rmdStartAge} are projected to increase your MAGI and may place you into a higher IRMAA tier.`
        });
    }
    ...
    const incomeDrop = vars.wages && vars.plannedRetirementAge
        ? vars.wages < 0.8 * (vars.wages ?? 0)
        : false;
    if ((vars.plannedRetirementAge && age === vars.plannedRetirementAge) ||
        incomeDrop) {
        insights.push({
...
export const IRMAA_INSIGHT_SCHEMA = [
    {
