 ...
 */
export default function NumberInput({ label, value, onChange, placeholder = "", error, min, max, step = 1, disabled = false, className = "", }) {
    return (_jsx(InputField, { label: label, value: String(value), onChange: (v) => {
            // Convert raw string to number safely
            const n = Number(v);
            onChange(Number.isFinite(n) ? n : 0);
        }, placeholder: placeholder, error: error, type: "number", disabled: disabled, className: className, 
