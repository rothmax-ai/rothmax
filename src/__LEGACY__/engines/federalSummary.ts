...
export function computeFederalSummary(
...
const input = state; // It's already normalized!
  ...
  //
  const tax: TaxableIncomeResult = computeTaxableIncome(input);
  ...
  //
  const ltcg = computeLTCGStacking(
    input.filingStatus,
    tax.taxableIncome,
    input.netInvestmentIncome  // temporary placeholder — real LTCG input later
  );
