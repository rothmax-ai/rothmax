// src/engines/brackets.ts
import type { BracketResult, BracketTable, FilingStatus } from "./types";
...
export function computeBracket(filingStatus: FilingStatus, taxableIncome: number): BracketResult {
  const brackets = BRACKETS_2025[filingStatus];

  const bracket = brackets.find(
    (b) => taxableIncome >= b.lower && taxableIncome < b.upper
  )!;
